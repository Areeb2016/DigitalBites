...
// getData = async()=>{
//   const response = await fetch('http://192.168.1.7:3000/viewratings');
...
// }
  ...
  saveData=async() => {
    ...
    fetch('https://digitalbites.herokuapp.com/restaurant/product/'+res,{
      method:"POST",
      headers: new Headers({
        Authorization:"Bearer "+token,
  ...
  render() {
    return (
        ...
        <Left>
          ...
          <Card title="RATE?" containerStyle={styles.card} style={{borderColor:'', borderRadius:30, borderBottomRightRadius:30, borderBottomLeftRadius:30}}>
...
<Text style={{color:'white', marginTop:10}}>Previous Ratings</Text>
   ...
   renderItem = {({item}) =>
    ...
    <AlertPro
          ...
          title="Rate Now"
          message="You are about to rate this restaurant"
          textCancel="Cancel"
          textConfirm="Confirm"
